...
On the CP, add each GPU's public key to `/etc/wireguard/wg0.conf`. Each GPU listens on its own WireGuard port (default `29817`, set in `gpu-wireguard-setup.sh`), **not** `51820`, so set the peer's `Endpoint` to the GPU's public IP + WG port:
...
CP config with two GPU peers. Each GPU must use a distinct `ListenPort` in the Trooper AI allowed inbound range (`29817–29836`), and the CP peer `Endpoint` must match:
...
**Symptom** — `ping -c 3 10.10.0.2` from the CP returns `Destination Host Unreachable`; on the GPU node `sudo wg show wg0` shows `transfer: 0 B received, <n> B sent` (handshake replies never arrive).

**Cause** — the GPU node's WireGuard daemon is listening on a port outside the Trooper AI allowed inbound range (e.g. an ephemeral port like `37460`), so the CP's replies are dropped by the external firewall.

**Fix**

1. Pin the GPU node's WG listen port inside `29817–29836` in `/etc/wireguard/wg0.conf`:
   ```ini
   [Interface]
   Address = 10.10.0.2/24
   ListenPort = 29817
   ```
2. Restart the tunnel on the GPU node:
   ```bash
   sudo systemctl restart wg-quick@wg0
   ```
3. On the CP, make sure the peer has `Endpoint = <GPU_PUBLIC_IP>:29817`, then restart `wg-quick@wg0`.
4. Verify:
   ```bash
   # GPU node — received bytes should now grow
   sudo wg show wg0
