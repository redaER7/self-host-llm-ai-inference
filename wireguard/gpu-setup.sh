...
# WireGuard client setup for a GPU node (Vast.ai or similar).
# Run after cp-setup.sh on the CP node.
#
# Usage:
#   export CP_NODE_IP=89.167.109.193
#   export WG_ADDRESS=10.8.0.2/24        # optional, defaults to 10.8.0.2/24
...
sudo apt-get update -y && sudo apt-get install -y wireguard-tools resolvconf
...
echo "Then restart WireGuard on the CP: sudo systemctl restart wg-quick@wg0"
echo ""
...
# Return route for VXLAN / hostNetwork traffic back to CP
PostUp = ip route add $CP_NODE_IP/32 via 10.8.0.1 dev wg0
...
echo ""
echo "On the CP node, add the VXLAN route:"
echo "   sudo ip route add <gpu-vast-internal-ip>/32 via ${WG_ADDRESS%/*} dev wg0"
echo ""
echo "The return route (GPU → CP) was added automatically via PostUp."
echo ""
