import asyncio, aiohttp, json, os, random, sys, time, statistics

MODEL =  "Qwen3.8-27B" #"Qwen/Qwen3.6-35B-A3B-FP8"
        ...
        async with session.post(URL, json=p) as resp:
            ...
            while True:
                ...
                if line.startswith(b"data:"):
                    ...
                    break
            if ttft is None and not saw_event:
                return {
                    "idx": idx,
                    "ttft": None,
                    "status": status,
                    "error": f"no SSE events (status {status})",
                }
            return {
                "idx": idx,
                "ttft": ttft,
...
async def main():
    token = os.environ.get("HETZNER_INFERENCE")
    if not token:
        print("ERROR: set HETZNER_INFERENCE env var (Bearer token for Hetzner Inference).")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector, headers=headers) as session:
        ...
        for r in ttft_results:
            s = f"{r['ttft']:.2f}s" if r["ttft"] and r["status"] == 200 else f"ERR({r['error'] or r['status']})"
