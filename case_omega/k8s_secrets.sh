...
# Create all Kubernetes secrets for case_omega.
...
for ns in omega cert-manager envoy-ai-gateway-system envoy-gateway-system kserve; do
  kubectl create namespace "$ns" --dry-run=client -o yaml | kubectl apply -f -
done

echo "[1/3] HuggingFace token (model download)"
