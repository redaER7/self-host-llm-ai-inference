...
echo "  sudo iptables -t nat -A PREROUTING -p tcp --dport 443 -j REDIRECT --to-port 30080"
echo "  sudo iptables -t nat -A OUTPUT -p tcp --dport 443 -j REDIRECT --to-port 30080"
echo "  sudo apt install -y iptables-persistent && sudo netfilter-persistent save"
